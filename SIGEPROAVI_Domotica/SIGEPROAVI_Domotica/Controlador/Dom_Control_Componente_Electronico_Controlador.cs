...
{
    public class Dom_Control_Componente_Electronico_Controlador
    {
        ...
        public static List<Dom_Control_Componente_ElectronicoDTO> ListarControlComponenteElectronico()
        {
            var request = new RestRequest("Dom_Control_Componente_Electronico", Method.GET);
            request.RequestFormat = DataFormat.Json;

            //request.AddParameter("Gpr_Galpon", request.JsonSerializer.Serialize(gpr_Galpon));
            //request.AddBody(gpr_Galpon);

            var response = client.Execute<List<Dom_Control_Componente_ElectronicoDTO>>(request);
